...
uthread_mutex_t itemLock;
uthread_cond_t canAddItem, canRemoveItem;
...
void* producer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_mutex_lock(itemLock);
    while(items >= MAX_ITEMS) {
      uthread_cond_wait(canAddItem);
    }
    items += 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;
    uthread_cond_signal(canRemoveItem);
    uthread_mutex_unlock(itemLock);
...
void* consumer (void* v) {
  for (int i=0; i<NUM_ITERATIONS; i++) {
    uthread_mutex_lock(itemLock);
    while (items <= 0) {
      uthread_cond_wait(canRemoveItem);
    }
    items -= 1;
    assert (items >= 0 && items <= MAX_ITEMS);
    histogram [items] ++;  
    uthread_cond_signal(canAddItem);
    uthread_mutex_unlock(itemLock);
...
int main (int argc, char** argv) {
  ...
  itemLock = uthread_mutex_create();
  canAddItem = uthread_cond_create(itemLock);
  canRemoveItem = uthread_cond_create(itemLock);
  ...
  uthread_mutex_destroy(itemLock);
  uthread_cond_destroy(canAddItem);
  uthread_cond_destroy(canRemoveItem);
}
